...
function normalizeSettings(input: Partial<Settings> | null | undefined): Settings {
  ...
  return {
    ...
    ai_primary_model: ['gemini-2.5-flash', 'gemini-2.5-pro'].includes(String(value.ai_primary_model || '').trim()) ? (String(value.ai_primary_model || '').trim() === 'gemini-2.5-pro' ? 'gemini-3.1-pro-preview' : CURRENT_GEMINI_MODEL) : String(value.ai_primary_model || DEFAULT_SETTINGS.ai_primary_model).trim() || DEFAULT_SETTINGS.ai_primary_model,
    ...
    ai_fallback_1_model: ['gemini-2.5-flash', 'gemini-2.5-pro'].includes(String(value.ai_fallback_1_model || '').trim()) ? (String(value.ai_fallback_1_model || '').trim() === 'gemini-2.5-pro' ? 'gemini-3.1-pro-preview' : CURRENT_GEMINI_MODEL) : String(value.ai_fallback_1_model || DEFAULT_SETTINGS.ai_fallback_1_model).trim() || DEFAULT_SETTINGS.ai_fallback_1_model,
    ...
    ai_fallback_2_model: ['gemini-2.5-flash', 'gemini-2.5-pro'].includes(String(value.ai_fallback_2_model || '').trim()) ? (String(value.ai_fallback_2_model || '').trim() === 'gemini-2.5-pro' ? 'gemini-3.1-pro-preview' : CURRENT_GEMINI_MODEL) : String(value.ai_fallback_2_model || DEFAULT_SETTINGS.ai_fallback_2_model).trim() || DEFAULT_SETTINGS.ai_fallback_2_model,
