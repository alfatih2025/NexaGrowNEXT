...
export function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      await login();
      // Setelah login berhasil, onAuthStateChanged akan mendeteksi user
      // dan App.tsx akan otomatis render halaman utama
    } catch (err: any) {
        ...
        <div className="glass-card p-8 bg-white/80 dark:bg-slate-900/80 shadow-2xl relative z-10 border border-slate-200/50 dark:border-slate-800/50">
          ...
          <div className="space-y-5">
            ...
            <button
              onClick={handleLogin}
              disabled={isLoading}
              className="w-full py-3 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-white font-semibold rounded-xl shadow-sm transition-all active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
